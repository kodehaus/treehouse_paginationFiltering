...
function searchData (displayPage) {
  ...
  if (searchTerm) {
    for (let i = 0; i < studentDataList.length; i++) {
      const firstNameMatch = studentDataList[i].name.first.toLowerCase().indexOf(searchTerm.toLowerCase());
      const lastNameMatch = studentDataList[i].name.last.toLowerCase().indexOf(searchTerm.toLowerCase());
      if ((+firstNameMatch !== -1) || (+lastNameMatch !== -1)) {
        filteredStudentList.push(studentDataList[i]);
      }
    }
  } else {
    filteredStudentList = studentDataList;
  }
...
function clearError () {
  const clearPreviousErr = document.getElementById('error');
  if (clearPreviousErr) {
...
linkListUL.addEventListener('click', (event) => {
  ...
  if (elem.tagName === 'BUTTON') { searchData(+elem.textContent); }
});
